...
export default function FoodCard({ item }: FoodCardProps) {
  ...
  return (
    <div
      ...
      style={{
        background: item.available ? '#ffffff' : '#1a2340',
        border: item.available ? '1px solid rgba(212, 175, 55, 0.15)' : '1px solid rgba(255,255,255,0.05)',
      }}
    >
      {/* Image */}
      ...
      <div className="flex flex-col gap-1.5 p-3 flex-1">
        <h3
          className="text-xs sm:text-sm font-bold text-[#0a1628] leading-tight"
        ...
        {quantity === 0 ? (
          <motion.button
            ...
            style={{
              ...
              color: item.available ? '#0a1628' : '#555',
              fontFamily: 'Georgia, serif',
            }}
          >
          ...
          <div className="flex items-center justify-between rounded-lg overflow-hidden" style={{ background: '#0a1628' }}>
            <button
      ...
      <AnimatePresence>
        {isAdding && (
          ...
          >
            <div className="px-3 py-1.5 rounded-full text-xs font-bold" style={{ background: '#d4af37', color: '#0a1628' }}>
              Added!
            </div>
          </motion.div>
        )}
